 ...
 *
 * @param m Pointer to the MainWindow. Client will use this to communicate with the interface
 */
Client::Client(MainWindow *m) {
    // Set private Screen reference variable (mw) to the m
    mw = m;
 ...
 */
bool Client::attemptConnect(string ip, int p) {
    ipstr = ip;
    port = p;
    
    // Initialize the socket
    if(!initSocket()) {
        mw->printLine("Could not initialize socket");
        return false;
    }
    ...
    loginPkt->setUsername(clientName);
    sendData(loginPkt);
    delete loginPkt;
...
void Client::clientProcess() {
    ...
    if(lenRecv == 0) {
        ...
        mw->printLine("Socket closed by server, disconnecting\n");
        clientRunning = false;
    } else if(lenRecv == -1) {
...
void Client::handleRequest(Packet *pkt) {
    ...
    case OP(CHAT): {
        ...
        chatPkt->parse();
        mw->printLine(chatPkt->getName() + ": " + chatPkt->getMessage());
        delete chatPkt;
    }
        break;
    case OP(DC): {
        ...
        mw->printLine("Disonnected by Server: " + dcPkt->getReason());
...
void Client::sendData(Packet *pkt){
    ...
    while(totalSent < dataLen) {
        ...
        // Server closed the connection
        if(n < 0) {
            mw->printLine("Error in sending data, socket closed, disconnecting\n");
            clientRunning = false;
            break;
        }
